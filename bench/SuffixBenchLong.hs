...
main = do
    defaultMain [ bgroup "suffixes-short"
                      ...
                      , bench "suffix (zipper)" $ nf suffixList shortString
                ...
                , bgroup "suffixes-unicode"
                      [ bench "suffix (paramorphism)" $ nf suffix unicodeString
                      , bench "suffix (pattern matching)" $ nf suffixPattern unicodeString
                      , bench "suffix (pattern matching v2)" $ nf suffixPattern2 unicodeString
                      , bench "suffix (pattern matching v3)" $ nf suffixPattern3 unicodeString
                      , bench "suffix (list comprehension)" $ nf suffixList unicodeString
                      , bench "suffix (zipper)" $ nf suffixList unicodeString
                      , bench "suffix (functoriality)" $ nf suffixFunctor unicodeString
                      , bench "suffix (pattern matching done right)" $ nf suffixPattern4 unicodeString
